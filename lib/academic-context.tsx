...
export function AcademicProvider({ children }: { children: ReactNode }) {
  ...
  // Pre-compute O(1) sets for completed and in-progress courses
  // This avoids O(N) Array.includes() calls inside loops and frequent callbacks
  const currentCompletedCourses = profile.progress[profile.major]?.completedCourses;
  const completedSet = useMemo(() => new Set(currentCompletedCourses || []), [currentCompletedCourses]);

  const currentInProgressCourses = profile.progress[profile.major]?.inProgressCourses;
  const inProgressSet = useMemo(() => new Set(currentInProgressCourses || []), [currentInProgressCourses]);
  ...
  }, [courseMap, completedSet, profile]);
  ...
  const getCourseStatus = useCallback((courseId: string): CourseStatus => {
    ...
    return 'locked';
  }, [completedSet, inProgressSet, arePrereqsMet, profile]);
  ...
  const getPrerequisiteChain = useCallback((courseId: string): CourseWithPrereqs[][] => {
    ...
    while (currentLevel.length > 0) {
      ...
      for (const id of currentLevel) {
        ...
        const missingPrereqs = course.prerequisites.filter(pid => !currentProgress || !completedSet.has(pid));

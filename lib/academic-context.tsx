...
export function AcademicProvider({ children }: { children: ReactNode }) {
  ...
  const getPrerequisiteChain = useCallback((courseId: string): CourseWithPrereqs[][] => {
    ...
    // Performance Optimization: Pre-compute completed courses Set for O(1) lookups
    const completedSet = new Set<string>(currentProgress?.completedCourses ?? []);

    while (currentLevel.length > 0) {
      const nextLevel: string[] = [];
      const levelCourses: CourseWithPrereqs[] = [];
      // Performance Optimization: Prevent duplicate processing in diamond dependency structures

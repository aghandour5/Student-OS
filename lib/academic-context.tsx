...
export function AcademicProvider({ children }: { children: ReactNode }) {
  ...
  const getPrerequisiteChain = useCallback((courseId: string): CourseWithPrereqs[][] => {
    ...
    const currentProgress = profile.progress[profile.major];
    // ⚡ Bolt Optimization: Pre-compute completed courses into a Set for O(1) lookups in the inner loop
    const completedSet = new Set(currentProgress?.completedCourses || []);

    while (currentLevel.length > 0) {
      const nextLevel: string[] = [];
      // ⚡ Bolt Optimization: Prevent duplicate nodes from diamond dependencies within the same BFS level

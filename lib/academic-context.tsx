...
export function AcademicProvider({ children }: { children: ReactNode }) {
  ...
  const currentProgressOpt = profile.progress[profile.major];
  const completedCoursesOpt = currentProgressOpt?.completedCourses;
  const inProgressCoursesOpt = currentProgressOpt?.inProgressCourses;

  // O(1) completed course lookup used heavily by context logic
  const completedSet = useMemo(() => {
    return new Set(completedCoursesOpt ?? []);
  }, [completedCoursesOpt]);

  // O(1) in-progress course lookup used heavily by context logic
  const inProgressSet = useMemo(() => {
    return new Set(inProgressCoursesOpt ?? []);
  }, [inProgressCoursesOpt]);
  ...
  const getCourseStatus = useCallback((courseId: string): CourseStatus => {
    ...
    return 'locked';
  }, [profile, completedSet, inProgressSet, arePrereqsMet]);

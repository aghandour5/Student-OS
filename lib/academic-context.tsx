...
export function AcademicProvider({ children }: { children: ReactNode }) {
  ...
  const completedCoursesList = profile.progress[profile.major]?.completedCourses;
  const inProgressCoursesList = profile.progress[profile.major]?.inProgressCourses;

  const completedCoursesSet = useMemo(() => {
    return new Set(completedCoursesList || []);
  }, [completedCoursesList]);

  const inProgressCoursesSet = useMemo(() => {
    return new Set(inProgressCoursesList || []);

...
export function AcademicProvider({ children }: { children: ReactNode }) {
  ...
  const getPrerequisiteChain = useCallback((courseId: string): CourseWithPrereqs[][] => {
    ...
    const currentProgress = profile.progress[profile.major];
    // Bolt Optimization: Pre-compute O(1) set to prevent O(N) .includes() lookups in nested BFS loops
    ...
    while (currentLevel.length > 0) {
      ...
      // Bolt Optimization: Track uniqueness within current breadth layer to stop diamond dependency duplication
      ...
      for (const id of currentLevel) {
        ...
        // Bolt Optimization: O(1) lookup reduces processing from O(N^2) to O(N) for deep trees

...
export function AcademicProvider({ children }: { children: ReactNode }) {
  ...
  // Extract arrays to local scope to improve React rendering performance
  ...
  // Derive O(1) Sets for optimized lookups
  const completedCoursesSet = useMemo(() => new Set(completedCourses), [completedCourses]);
  const inProgressCoursesSet = useMemo(() => new Set(inProgressCourses), [inProgressCourses]);
  ...
  const getCourseNote = useCallback((courseId: string): string => {
    return notes?.find(n => n.courseId === courseId)?.note ?? '';
  }, [notes]);

...
export function AcademicProvider({ children }: { children: ReactNode }) {
  ...
  const getPrerequisiteChain = useCallback((courseId: string): CourseWithPrereqs[][] => {
    ...
    // Performance optimization: Pre-compute a Set for O(1) lookups instead of O(N) array .includes()
    ...
    while (currentLevel.length > 0) {
      ...
      // Performance optimization: Use a local Set to prevent redundant processing of the same node
      // in diamond dependency structures within a single breadth level execution.

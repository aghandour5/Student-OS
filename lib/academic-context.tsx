...
export function AcademicProvider({ children }: { children: ReactNode }) {
  ...
  const currentProgressCompleted = profile.progress[profile.major]?.completedCourses;
  const completedCoursesSet = useMemo(() => new Set(currentProgressCompleted || []), [currentProgressCompleted]);

  const currentProgressInProgress = profile.progress[profile.major]?.inProgressCourses;
  const inProgressCoursesSet = useMemo(() => new Set(currentProgressInProgress || []), [currentProgressInProgress]);

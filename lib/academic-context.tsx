...
export function AcademicProvider({ children }: { children: ReactNode }) {
  ...
  // Extract optional properties outside of the useMemo dependency array
  // to prevent unnecessary re-evaluations and satisfy lint rules.
  const currentCompletedCourses = profile.progress[profile.major]?.completedCourses;
  const currentInProgressCourses = profile.progress[profile.major]?.inProgressCourses;

  // O(1) lookups for student progress to avoid O(N^2) Array.includes bottlenecks
  const completedCoursesSet = useMemo(() => {
    return new Set(currentCompletedCourses || []);
  }, [currentCompletedCourses]);

  const inProgressCoursesSet = useMemo(() => {
    return new Set(currentInProgressCourses || []);
  }, [currentInProgressCourses]);
  ...
  const completedCredits = useMemo(() => {
    if (!currentCompletedCourses) return 0;

    return currentCompletedCourses.reduce((sum, cid) => {
      const course = courseMap.get(cid);
      return sum + (course?.credits ?? 0);
    }, 0);
  }, [currentCompletedCourses, courseMap]);

  const inProgressCredits = useMemo(() => {
    if (!currentInProgressCourses) return 0;

    return currentInProgressCourses.reduce((sum, cid) => {
      const course = courseMap.get(cid);
      return sum + (course?.credits ?? 0);
    }, 0);
  }, [currentInProgressCourses, courseMap]);

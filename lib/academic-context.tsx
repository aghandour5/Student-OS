...
export function AcademicProvider({ children }: { children: ReactNode }) {
  ...
  const completedCoursesArray = profile.progress[profile.major]?.completedCourses;
  const completedSet = useMemo(() => new Set(completedCoursesArray ?? []), [completedCoursesArray]);

  const inProgressCoursesArray = profile.progress[profile.major]?.inProgressCourses;

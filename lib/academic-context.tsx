...
export function AcademicProvider({ children }: { children: ReactNode }) {
  ...
  // Optional chaining extraction to satisfy exhaustive-deps linting rules
  ...
  // O(1) Sets for course status lookups to prevent O(N^2) renders in lists
  const completedCoursesSet = useMemo(() => {
    return new Set(completedCourses || []);
  }, [completedCourses]);

  const inProgressCoursesSet = useMemo(() => {
    return new Set(inProgressCourses || []);
  }, [inProgressCourses]);

...
export function AcademicProvider({ children }: { children: ReactNode }) {
  ...
  // Extract arrays before useMemo to satisfy eslint-plugin-react-hooks/exhaustive-deps
  const currentCompleted = profile.progress[profile.major]?.completedCourses;
  const currentInProgress = profile.progress[profile.major]?.inProgressCourses;

  // Derive Sets for O(1) lookups
  const completedCoursesSet = useMemo(() => new Set(currentCompleted || []), [currentCompleted]);
  const inProgressCoursesSet = useMemo(() => new Set(currentInProgress || []), [currentInProgress]);

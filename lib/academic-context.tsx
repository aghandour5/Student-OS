...
export function AcademicProvider({ children }: { children: ReactNode }) {
  ...
  // Memoize O(1) lookups for completed and in-progress courses
  const currentCompletedArr = profile.progress[profile.major]?.completedCourses;
  const completedCoursesSet = useMemo(() => new Set(currentCompletedArr || []), [currentCompletedArr]);

  const currentInProgressArr = profile.progress[profile.major]?.inProgressCourses;
  const inProgressCoursesSet = useMemo(() => new Set(currentInProgressArr || []), [currentInProgressArr]);
  ...
  const arePrereqsMet = useCallback((courseId: string): boolean => {
    ...
    if (completedCoursesSet.size === 0) return false;
  ...
  const getMissingPrereqs = useCallback((courseId: string): CourseWithPrereqs[] => {
    const course = courseMap.get(courseId);
    if (!course) return [];
    if (completedCoursesSet.size === 0) return course.prerequisites.map(pid => courseMap.get(pid)).filter(Boolean) as CourseWithPrereqs[];
  ...
  const completedCredits = useMemo(() => {
    let sum = 0;
    completedCoursesSet.forEach(cid => {
  ...
  const inProgressCredits = useMemo(() => {
    let sum = 0;
    inProgressCoursesSet.forEach(cid => {
      const course = courseMap.get(cid);
      sum += course?.credits ?? 0;
    });
    return sum;

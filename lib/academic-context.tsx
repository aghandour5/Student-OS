...
export function AcademicProvider({ children }: { children: ReactNode }) {
  ...
  const completedCoursesArr = profile.progress[profile.major]?.completedCourses;
  const inProgressCoursesArr = profile.progress[profile.major]?.inProgressCourses;

  const completedSet = useMemo(() => new Set(completedCoursesArr || []), [completedCoursesArr]);
  const inProgressSet = useMemo(() => new Set(inProgressCoursesArr || []), [inProgressCoursesArr]);

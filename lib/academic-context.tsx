...
export function AcademicProvider({ children }: { children: ReactNode }) {
  ...
  // Derive sets for O(1) lookups in academic logic callbacks
  const currentProgress = profile.progress[profile.major];
  const completedCoursesList = currentProgress?.completedCourses;
  const completedCoursesSet = useMemo(() => new Set(completedCoursesList || []), [completedCoursesList]);

  const inProgressCoursesList = currentProgress?.inProgressCourses;
  const inProgressCoursesSet = useMemo(() => new Set(inProgressCoursesList || []), [inProgressCoursesList]);
  ...
  const arePrereqsMet = useCallback((courseId: string): boolean => {
    ...
    if (!completedCoursesSet) return false;
  ...
  const getMissingPrereqs = useCallback((courseId: string): CourseWithPrereqs[] => {
    const course = courseMap.get(courseId);
    if (!course) return [];
    if (!completedCoursesSet) return course.prerequisites.map(pid => courseMap.get(pid)).filter(Boolean) as CourseWithPrereqs[];
  ...
  const getPrerequisiteChain = useCallback((courseId: string): CourseWithPrereqs[][] => {
    ...
    while (currentLevel.length > 0) {
      ...
      for (const id of currentLevel) {
        ...
        const missingPrereqs = course.prerequisites.filter(pid => !completedCoursesSet || !completedCoursesSet.has(pid));

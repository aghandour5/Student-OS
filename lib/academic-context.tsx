...
export function AcademicProvider({ children }: { children: ReactNode }) {
  ...
  }, [courseMap, completedCoursesArray, completedSet]);
  ...
  const getCourseStatus = useCallback((courseId: string): CourseStatus => {
    if (!completedCoursesArray) return 'locked';
    ...
    return 'locked';
  }, [completedCoursesArray, completedSet, inProgressSet, arePrereqsMet]);
  ...
  }, [courses, completedCoursesArray, completedSet]);
  ...
  const getPrerequisiteChain = useCallback((courseId: string): CourseWithPrereqs[][] => {
    ...
    while (currentLevel.length > 0) {
      ...
      for (const id of currentLevel) {
        ...
        const missingPrereqs = course.prerequisites.filter(pid => !completedCoursesArray || !completedSet.has(pid));

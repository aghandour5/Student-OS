...
export function AcademicProvider({ children }: { children: ReactNode }) {
  ...
  // Extract optional chains before useMemo to satisfy exhaustive-deps and prevent
  // recreating sets when unrelated profile properties (like notes) change.
  ...
  const getPrerequisiteChain = useCallback((courseId: string): CourseWithPrereqs[][] => {
    ...
    while (currentLevel.length > 0) {
      ...
      const nextLevelSet = new Set<string>(); // Prevent duplicate additions on the exact same BFS level

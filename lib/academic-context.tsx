...
export function AcademicProvider({ children }: { children: ReactNode }) {
  ...
  // Extract granular progress data to stabilize dependencies
  const currentMajor = profile.major;
  const currentProgress = profile.progress[currentMajor];
  const completedCourses = currentProgress?.completedCourses ?? [];
  const inProgressCourses = currentProgress?.inProgressCourses ?? [];
  ...
  const arePrereqsMet = useCallback((courseId: string): boolean => {
    ...
    return course.prerequisites.every(pid => completedCourses.includes(pid));
  ...
  const getCourseStatus = useCallback((courseId: string): CourseStatus => {
    ...
    if (inProgressCourses.includes(courseId)) return 'in_progress';
    if (arePrereqsMet(courseId)) return 'available';
    return 'locked';
  }, [completedCourses, inProgressCourses, arePrereqsMet]);
  ...
  const getMissingPrereqs = useCallback((courseId: string): CourseWithPrereqs[] => {
    ...
    return course.prerequisites
      .filter(pid => !completedCourses.includes(pid))
  ...
  const isYearCompleted = useCallback((year: number): boolean => {
    ...
    return yearCourses.every(c => completedCourses.includes(String(c.id)));
  ...
  const calculateSemesterGPA = useCallback((courseIds: string[]): number => {
    const semesterGrades = grades.filter(g => courseIds.includes(g.courseId));
  ...
  const completedCredits = useMemo(() => {
    return completedCourses.reduce((sum, cid) => {
  ...
  const inProgressCredits = useMemo(() => {
    return inProgressCourses.reduce((sum, cid) => {
      const course = courseMap.get(cid);
      return sum + (course?.credits ?? 0);
    }, 0);
  }, [inProgressCourses, courseMap]);
  ...
  const getPrerequisiteChain = useCallback((courseId: string): CourseWithPrereqs[][] => {
    ...
    while (currentLevel.length > 0) {
      ...
      for (const id of currentLevel) {
        ...
        const missingPrereqs = course.prerequisites.filter(pid => !completedCourses.includes(pid));

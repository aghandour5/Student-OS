...
export function AcademicProvider({ children }: { children: ReactNode }) {
  ...
  const completedCoursesArr = currentMajorProgress?.completedCourses;
  const inProgressCoursesArr = currentMajorProgress?.inProgressCourses;
  ...
  const getPrerequisiteChain = useCallback((courseId: string): CourseWithPrereqs[][] => {
    ...
    while (currentLevel.length > 0) {
      ...
      const nextLevelSet = new Set<string>(); // Prevent duplicate additions to the same level

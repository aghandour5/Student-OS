...
export function AcademicProvider({ children }: { children: ReactNode }) {
  ...
  // Use useMemo to extract arrays to ensure stable references even if progress[major] is undefined
  const completedCoursesArray = useMemo(() => profile.progress[profile.major]?.completedCourses || [], [profile.progress, profile.major]);
  const inProgressCoursesArray = useMemo(() => profile.progress[profile.major]?.inProgressCourses || [], [profile.progress, profile.major]);
  ...
  const completedCredits = useMemo(() => {
    return Array.from(completedCoursesSet).reduce((sum, cid) => {
      const course = courseMap.get(cid);
      return sum + (course?.credits ?? 0);
    }, 0);
  }, [completedCoursesSet, courseMap]);

  const inProgressCredits = useMemo(() => {
    return Array.from(inProgressCoursesSet).reduce((sum, cid) => {
      const course = courseMap.get(cid);
      return sum + (course?.credits ?? 0);
    }, 0);
  }, [inProgressCoursesSet, courseMap]);

...
export function AcademicProvider({ children }: { children: ReactNode }) {
  ...
  const completedCredits = useMemo(() => {
    if (!completedCoursesArr) return 0;

    return completedCoursesArr.reduce((sum, cid) => {
      const course = courseMap.get(cid);
      return sum + (course?.credits ?? 0);
    }, 0);
  }, [completedCoursesArr, courseMap]);

  const inProgressCredits = useMemo(() => {
    if (!inProgressCoursesArr) return 0;

    return inProgressCoursesArr.reduce((sum, cid) => {
      const course = courseMap.get(cid);
      return sum + (course?.credits ?? 0);
    }, 0);
  }, [inProgressCoursesArr, courseMap]);

...
export function AcademicProvider({ children }: { children: ReactNode }) {
  ...
  // O(1) lookups for current progress state
  const completedSet = useMemo(() => {
    const currentProgress = profile.progress[profile.major];
    return new Set(currentProgress ? currentProgress.completedCourses : []);
  }, [profile.major, profile.progress]);

  const inProgressSet = useMemo(() => {
    const currentProgress = profile.progress[profile.major];
    return new Set(currentProgress ? currentProgress.inProgressCourses : []);
  }, [profile.major, profile.progress]);

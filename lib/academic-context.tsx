...
export function AcademicProvider({ children }: { children: ReactNode }) {
  ...
  const completedSet = useMemo(() => {
    const currentP = profile.progress[profile.major];
    return new Set(currentP ? currentP.completedCourses : []);
  }, [profile.progress, profile.major]);

  const inProgressSet = useMemo(() => {
    const currentP = profile.progress[profile.major];
    return new Set(currentP ? currentP.inProgressCourses : []);
  }, [profile.progress, profile.major]);
  ...
  const getCourseStatus = useCallback((courseId: string): CourseStatus => {
    ...
    return 'locked';
  }, [completedSet, inProgressSet, profile, arePrereqsMet]);

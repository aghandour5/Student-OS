...
export function AcademicProvider({ children }: { children: ReactNode }) {
  ...
  // Extract variables for safe memoization of Sets
  ...
  // Convert to O(1) Sets for significantly faster lookups during list renders
  const completedCoursesSet = useMemo(() => new Set(completedCoursesArr || []), [completedCoursesArr]);
  const inProgressCoursesSet = useMemo(() => new Set(inProgressCoursesArr || []), [inProgressCoursesArr]);
  ...
  const arePrereqsMet = useCallback((courseId: string): boolean => {
    ...
    if (!completedCoursesArr) return false;
    return course.prerequisites.every(pid => completedCoursesSet.has(pid));
  }, [courseMap, completedCoursesArr, completedCoursesSet]);
  ...
  const getCourseStatus = useCallback((courseId: string): CourseStatus => {
    if (!completedCoursesArr) return 'locked';
    ...
    return 'locked';
  }, [completedCoursesArr, completedCoursesSet, inProgressCoursesSet, arePrereqsMet]);
  ...
  const getMissingPrereqs = useCallback((courseId: string): CourseWithPrereqs[] => {
    const course = courseMap.get(courseId);
    if (!course) return [];
    if (!completedCoursesArr) return course.prerequisites.map(pid => courseMap.get(pid)).filter(Boolean) as CourseWithPrereqs[];
  ...
  }, [courses, completedCoursesArr, completedCoursesSet]);
  ...
  const getPrerequisiteChain = useCallback((courseId: string): CourseWithPrereqs[][] => {
    ...
    while (currentLevel.length > 0) {
      ...
      const nextLevelSet = new Set<string>(); // Prevent duplicate nodes in the same level
      ...
      for (const id of currentLevel) {
        ...
        const missingPrereqs = course.prerequisites.filter(pid => !completedCoursesArr || !completedCoursesSet.has(pid));

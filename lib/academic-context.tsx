...
export function AcademicProvider({ children }: { children: ReactNode }) {
  ...
  // Performance Optimization: Pre-compute sets for O(1) lookups in academic logic callbacks
  const completedSet = useMemo(() => {
    const currentProgress = profile?.progress?.[profile?.major];
    return new Set<string>(currentProgress?.completedCourses || []);
  }, [profile?.progress, profile?.major]);

  const inProgressSet = useMemo(() => {
    const currentProgress = profile?.progress?.[profile?.major];
    return new Set<string>(currentProgress?.inProgressCourses || []);
  }, [profile?.progress, profile?.major]);
  ...
  const calculateSemesterGPA = useCallback((courseIds: string[]): number => {
    ...
    // Performance Optimization: Convert array includes to Set lookup for O(1) filtering
    const courseIdSet = new Set(courseIds);
    const semesterGrades = currentProgress.grades.filter(g => courseIdSet.has(g.courseId));
